...
//    p3.acc << 0.0,0.0,0;
    ...
    for(int i=0 ;i<2 ;i++){
        ...
        p2.pos<< 2,-1,0;
        p2.vel<< profile[profile.size()-1].vel[0],profile[profile.size()-1].vel[1],0;
        p2.acc<< profile[profile.size()-1].acc[0],profile[profile.size()-1].acc[1],0;
    ...
    path.push_back(segments(p2,p3,2.0));
    path.push_back(segments(p3,p1,2.0));

    profile = plan->get_profile(path,1.0,0.02);
//   for(int i=0 ;i< profile.size();i++){
//       std::cout << "step : " <<i<<std::endl;
//       std::cout <<     profile[i].vel[0]     <<std::endl;
//   }
